...
class NumpyGraphEncoder:
    ...
    def get_encoder(self, sid: str):
        ...
        if uid is not None and hasattr(self, f"{op_type}_{uid}"):
            return getattr(self, f"{op_type}_{uid}")
    ...
    def encode_ndarray(self, array, label):
        ...
        for i, dim in enumerate(array.shape):
            ...
            for j in range(dim):
                ...
                self.add_equality_edges(j, t, NodeValueType.DIM_VALUE)
    ...
    def get_graph_encoding(self, domain: List[Any], context: Mapping[str, Any], choice=None,
    ...
    def get_domain_node_id(self, domain, choice):
        found = False
        # first try fast equality test
        for i, x in enumerate(domain):
            if x is choice:
                choice_idx = i
                found = True
                break
        if not found:   #  then try slow but accurate equality test
            for i, x in enumerate(domain):
                if NumpyChecker.check(x, choice):
                    choice_idx = i
                    found = True
                    break
        assert found, f"InternalError: choice is not listed in domain. Domain: {domain}, Choice: {choice}"

        return self.label_to_node_id[f"D{choice_idx}"]


    def Select(self, domain: List[Any], context: Mapping[str, Any], choice=None,
               mode='training', **kwargs):
        graph = self.get_graph_encoding(domain, context, choice, mode)

        if mode == 'training':
            graph['choice'] = self.get_domain_node_id(domain, choice)
        else:
            graph['mapping'] = {self.label_to_node_id[f"D{i}"]: v 
                                for i, v in enumerate(domain)}
        return graph

    def SelectFixed(self, domain: List[Any], context: Mapping[str, Any], choice=None,
                    mode='training', **kwargs):
        graph = self.get_graph_encoding(domain, context, choice, mode)

        if mode == 'training':
            graph['choice'] = graph['domain'].index(self.get_domain_node_id(domain, choice))
            del graph['domain']
        else:
            graph['mapping'] = {i: v for i, v in enumerate(domain)}
        return graph

    def Sequence(self, domain: List[Any], context: Mapping[str, Any], choice=None,
                 mode='training', **kwargs):
        domain = list(domain) + [None]   # add terminal
        graph = self.get_graph_encoding(domain, context, choice, mode)

        terminal_id = graph['domain'][-1]

        if mode == 'training':
            graph['choice'] = [self.get_domain_node_id(domain, x) for x in choice] + [terminal_id]
        else:
            graph['mapping'] = {i: v for i, v in enumerate(domain)}

        graph['terminal'] = terminal_id

        return graph

    def SequenceFixed(self, domain: List[Any], context: Mapping[str, Any], choice=None,
                      mode='training', **kwargs):
        domain = list(domain) + [None]
        graph = self.get_graph_encoding(domain, context, choice, mode)

        terminal_id = len(domain) - 1

        if mode == 'training':
            graph['choice'] = [graph['domain'].index(self.get_domain_node_id(domain, x)) for x in choice] + [terminal_id]   # todo: remove index here
            del graph['domain']
        else:
            graph['mapping'] = {i: v for i, v in enumerate(domain)} # todo: improve this

        graph['terminal'] = terminal_id

        return graph

    def OrderedSubset(self, domain: List[Any], context: Mapping[str, Any], choice=None,
                      mode='training', **kwargs):
        return self.Sequence(domain, context, choice, mode, **kwargs)

    def Subset(self, domain: List[Any], context: Mapping[str, Any], choice=None,
               mode='training', **kwargs):
        # delete terminal token
        graph = self.Sequence(domain, context, choice, mode, **kwargs)
        graph['domain'].pop()
        if mode == 'training':
            graph['choice'].pop()
        return graph
