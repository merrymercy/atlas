...
def gen_traces(seqs, funcs, size_per_seq, max_seq_per_input):
    ...
    for seq, input_generator in seqs:
        ...
        while i < size_per_seq and error_ct < MAX_ERROR_RETRY:
            ...
            ct = 0
            for (output, prog), trace in numpy_sequence_generator\
                    .with_env(tracing=True, replay={"func_seq" : infinite_list(seq)})\
                    .generate(inputs, None, funcs=funcs):
                traces.append(trace)
    ...
    traces = [numpy_sequence_generator.with_env(replay=trace, tracing=True).call(*args)[1]
